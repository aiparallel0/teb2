...
HttpResp handle_nudge_create(HttpReq req, Ctx *ctx)
{
    ...
    if (q.message[0] == '\0') return json_error(400, "missing_message");
...
HttpResp handle_nudge_list(HttpReq req, Ctx *ctx)
{
    ...
    nr = list_nudges(ctx->db, q);
    if (nr.err == ERR_NOT_FOUND) return json_ok("{\"nudges\":[]}");
    if (nr.err != ERR_OK)        return json_error(500, "db_error");

    snprintf(buf, sizeof(buf),
             "{\"id\":%lld,\"message\":\"%s\"}",
             (long long)nr.nudge.id, nr.nudge.message);
    return json_ok(buf);
}
